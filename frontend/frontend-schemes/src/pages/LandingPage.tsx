...
import { Sprout, DropletIcon, Tractor, ArrowRight, Leaf } from 'lucide-react';
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-green-50 to-white">
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur border-b border-green-100 shadow-sm">
        <div className="max-w-6xl mx-auto px-4 py-3 pr-24 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <Leaf className="text-green-600" size={28} />
            <span className="text-xl sm:text-2xl font-bold text-green-800">{t('app_name')}</span>
          </div>
          <LanguageSelector />
        </div>
      </header>
      ...
      <section className="max-w-4xl mx-auto px-4 pt-16 pb-12 text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-4 justify-center items-center mb-4">
          <ListenButton size="md" text={`${t('hero_title')}. ${t('hero_sub')}`} className="!p-3" />
          <button
            onClick={() => navigate('/onboarding')}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {features.map(({ icon, key, descKey, onClick }) => (
            <button
              type="button"
              key={key}
              className="text-left bg-white rounded-2xl p-6 shadow-sm border border-gray-100 hover:shadow-md hover:border-green-300 transition-all cursor-pointer active:scale-[0.99]"
              onClick={onClick}
            >
